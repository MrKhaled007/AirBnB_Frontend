<template>
  <div v-if="isVisible" class="modal-overlay">
    <div class="modal">
      <h2>Dont Worry! Just Spend!</h2>
      <p>It will only cost you: €{{ cost }}</p>
    ...
    <div class="flip-card-inner">
      ...
      <div>
        ...
        <input v-model="cardNumber" id="cardNumber" type="text" />
      </div>
      <div>
...
export default {
  ...
  methods: {
    async log(){
      let result = await axios.get(`https://localhost:5001/User/GetUser?email=${this.email}&password=${this.password}`);
    ...
    generatePDF() {
      ...
      doc.text(`Total Cost: €${this.cost}`, 10, 20);
      doc.text(`Username: ${username}`, 10, 30);
      doc.text(`Email: ${email}`, 10, 40);
      doc.text(`Credit Card Number: ${this.cardNumber}`, 10, 50);
      
      
      doc.text('This is Your Ticket', 10, 70);
      doc.text(`Start of Stay Date: ${new Date(this.bookingDate).toLocaleDateString()}`, 10, 80);
      doc.text(`Leaving The Camp Date: ${new Date(this.leavingDate).toLocaleDateString()}`, 10, 90);

      // Add more details as needed
      doc.text('Thank you for your payment!', 10, 100);
